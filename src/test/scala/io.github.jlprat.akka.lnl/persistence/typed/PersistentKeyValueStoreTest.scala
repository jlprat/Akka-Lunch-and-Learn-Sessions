...
class PersistentKeyValueStoreTest
  ...
  "PersistentKeyValueStore" should "save key-value pairs" in {

    val persistentKVS = testKit.spawn(PersistentKeyValueStore("kvs-1"))
  ...
  it should "not persist the key if it exceeds the maximum of 100 chars" in {
    val persistentKVS = testKit.spawn(PersistentKeyValueStore("kvs-2"))
    val probePut      = testKit.createTestProbe[PutResponse]()
  ...
  it should "return missing when looking up for a missing key" in {
    val persistentKVS = testKit.spawn(PersistentKeyValueStore("kvs-3"))
    val probeGet      = testKit.createTestProbe[GetResponse]()
  ...
  it should "process commands one after the other when being persisted" in {
    ...
    val persistentKVS = testKit.spawn(PersistentKeyValueStore("kvs-4"))
    val probePut      = testKit.createTestProbe[PutResponse]()
    val probeGet      = testKit.createTestProbe[GetResponse]()
  ...
  it should "keep its state between restarts" in {
    val persistentKVS = testKit.spawn(PersistentKeyValueStore("kvs-5"))
    val probePut      = testKit.createTestProbe[PutResponse]()
    val probeGet      = testKit.createTestProbe[GetResponse]()
    ...
    val restartedKVS = testKit.spawn(PersistentKeyValueStore("kvs-5"))
